...
def generate_md_to_html(from_path, dest_path, template_path):
    template_html = ""
    markdown = ""
...
def generate_to_public(from_folder, to_folder, folder_dict, template_path):
    ...
    for fd in folder_dict.items():
        if isinstance(fd[1], dict):
            generate_to_public(os.path.join(from_folder, fd[0]), os.path.join(
                to_folder, fd[0]), fd[1], template_path)
        else:
            print(f"- generating HTML from - {os.path.join(
                from_folder, fd[0])}")
            generate_md_to_html(os.path.join(
                from_folder, fd[0]), os.path.join(to_folder, fd[0].replace(".md", ".html")), template_path)


def generate_pages_recursively(from_folder, to_folder, template_path):
    ...
    generate_to_public(from_folder, to_folder, folder_dict, template_path)
